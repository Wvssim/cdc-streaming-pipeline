...
| **S6** | Consolider, tester, préparer la soutenance (**tampon**) | Tests Testcontainers e2e, générateur de démo, sécurité JWT (si temps), README/diagrammes/rapport | **Scénario de démo répété avec succès + documentation finalisée** | ✅ terminée — T6.1 (tests e2e) à T6.6 (répétition de la démo) faits ; springdoc/Swagger ajouté sur les 6 services (écart rapport/code comblé) ; jalon final rejoué le 31/08/2026 sur base propre (latence upload→5 consommateurs ≈ 2,6 s) |
...
| # | Tâche | Est. | Dépend de | Definition of Done |
|---|---|---|---|---|
| **T5.1** | **`ocr-service`** : `@KafkaListener` → récupère le fichier depuis **MinIO** (via `storage_key`) → extraction **Apache Tika** (PDF/DOCX) → texte dans `ocr.extracted_text` (`doc_id, text, engine, extracted_at`) + `GET /api/ocr/{docId}`. Tesseract/tess4j (images) câblé (dispatch sur `content_type`), nécessite le binaire natif Tesseract installé sur la machine hôte pour être actif. | 3 ½j | Jalon S3 | Après upload d'un PDF, `GET /api/ocr/{docId}` renvoie le texte extrait. Après upload d'une image (Tesseract installé localement), `engine` vaut `tesseract`. |
...
| **T6.3** | **Sécurité basique** Spring Security + JWT (login + protection des endpoints) — **si le temps le permet** (Should). | 1 ½j | — | Les endpoints REST exigent un JWT valide ; le login renvoie un token. **✅ Fait** — `documents-api` émet le token (utilisateur unique en dur), les 6 services le valident avec le même secret partagé, frontend Angular branché (login, interceptor, guard, logout). Vérifié en conditions réelles le 31/08/2026 (front → 6 services) : mauvais mot de passe → 401, endpoint sans token → 403, login OK → token accepté par les 6 services. Un bug (403 vide au lieu de 401) trouvé et corrigé à cette occasion + test de non-régression `@WebMvcTest`. |
| **T6.4** | **README** (badge CI, schéma d'archi, instructions `cd infra && docker compose up`), **diagrammes** (flux, séquence) et **documentation technique** finalisés. | 1 ½j | — | Un lecteur externe peut démarrer le projet en suivant le README ; les diagrammes sont à jour. **✅ Fait** — README réécrit (ordre infra → connecteur → build → 6 services → front/curl, creds, captures), `docs/diagrammes.md` créé (5 diagrammes Mermaid : archi, séquence upload+fan-out, DLT, JWT, MCD ; validés au rendu), `docs/aide-memoire.md` remis à jour, structure/invariants d'ARCHITECTURE.md corrigés, scripts `infra/run-services.ps1`/`.sh`. springdoc-openapi 3.1.0 ajouté sur les 6 services (`/swagger-ui.html`, `/v3/api-docs`) — l'affirmation rapport/cahier des charges est désormais vraie. |
| **T6.5** | **Rapport PFE** finalisé (intègre cahier des charges, architecture, choix, résultats, perspectives). | 2 ½j | — | Le rapport est complet et relu. **✅ Fait** — section OpenAPI/Swagger ajoutée (+ capture `swagger_ui_documents_api.png`), RF-03 et tableau de stack ajustés (springdoc réel), section Tests Testcontainers étendue aux 5 consommateurs + DLT, section JWT étoffée (+ fix 401), critères d'acceptation et jalon final renseignés avec les chiffres réels de la répétition du 31/08. |
| **T6.6** | 🏁 **JALON** : **répétition du scénario de démonstration** de bout en bout (upload → 5 services → écrans Angular). | 1 ½j | Tout | La démo se déroule sans accroc, chronométrée, sur une base propre. **✅ Fait** — rejouée le 31/08/2026 sur base propre (`docker compose down -v`) : 6 conteneurs healthy, connecteur `RUNNING`, 6 services + frontend. Login JWT (401/403/OK), upload → événement CDC → 5 consumer groups (lag 0) → audit + notification (MailHog) + blockchain (`verify` valid:true) + OCR (Tika) + 3 règles SIEM ; message non-JSON → 5 entrées DLT, pipeline non bloqué. Latence upload→dernier consommateur ≈ 2,6 s en régime établi (< 5 s, RNF-05). |
